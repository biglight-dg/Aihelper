# AGENTS.md — Aihelper (Codex 담당)

> 이 프로젝트는 **Codex**가 담당한다. Codex는 이 파일(`AGENTS.md`)을 지침으로 읽는다.
> 이력: 구 `claudehelper` → 현 `C:\Users\chris\Codex\Projects\Aihelper` · GitHub: `biglight-dg/Aihelper`.
> 주의: GCP 프로젝트 식별자(`claudehelper-499608`)와 Google 공유드라이브 `data` 경로는 외부 자원이라 이름을 바꾸지 않는다.

---

# 나만의 지식 베이스 — AI 교육팀

외부 API 없는 로컬 지식 관리 도구. **Codex 세션이 팀장 역할을 하며 에이전트들을 오케스트레이션한다.**
...
| 에이전트 | 파일 | 역할 |
|----------|------|------|
| 팀장 | `agents/team_lead.py` | 사용자 요청 분석 + 작업 라우팅 (Codex = 팀장) |
...
2. Codex에 `"inbox 정리해줘"` 요청
3. Codex(팀장) → 교육자가 문서 작성 → QA 검토 → 큐레이터가 저장
4. 앱 **지식 베이스** 탭에서 결과 확인

### PPT 슬라이드 만들기 (Canva MCP)

1. inbox에 자료 입력 또는 지식베이스의 기존 문서 지정
2. Codex에 `"PPT 만들어줘"` 또는 `"슬라이드로 만들어줘"` 요청
...
- `sources.sync_expert_feeds()` — 유튜브인데 `feed_url` 미연결인 전문가를 일괄 RSS화(백필). **매주 월 09:15** 작업 스케줄러 `Aihelper-ExpertFeed`(`tools/expert_feed_sync.ps1`)가 호출 → 09:23 뉴스 브리핑 직전이라 그 회차 수집에 포함된다.
...
- **매주 자동 생성**: 자동화 담당 PC의 작업 스케줄러 `Aihelper-WeeklyDigest`가 매주 월요일 09:23에 `tools/weekly_digest.ps1`을 실행 → `codex exec`로 위 절차를 무인 수행. 로그: `logs/weekly_digest.log`. 3PC 중 한 대에만 등록한다.

### 유튜브 영상을 커리큘럼 참고자료로 넣기

1. Codex에 `"이 유튜브 [링크] 커리큘럼에 넣어줘"` 요청
...
1. Codex에 `"[툴/확장 링크] 보조 프로그램에 추가해줘"` 요청 (또는 앱 탭의 추가 폼)
...
1. Codex에 `"[내용] 꿀팁 추가해줘"` 요청 (또는 앱 탭의 추가 폼)
2. 팀장이 `tips_tools.add_tip(title, body, example, category, tags, source)` 호출
   - `category`: `Codex · 프롬프트 · 워크플로우 · ChatGPT·제미나이 · 자동화·MCP · 토큰 절약 · 일반`
...
Codex가 교육자 역할로 Canva MCP를 사용해 슬라이드를 생성한다.

```
1. educator.make_canva_outline(title, content) 으로 아웃라인 생성
2. 현재 Codex 세션에 연결된 Canva 도구로 디자인 생성
3. 생성된 디자인 URL/ID를 사용자에게 전달
4. 필요하면 Canva 편집 도구로 수정
```
...
- 캔버스는 1280×1600(4:5) 고정이며, 한 장에는 한 메시지만 담는다.
- 교재 문단을 복사하지 않고 핵심 개념을 최대 3개의 시각 카드로 압축한다.
- 표는 최대 4열×6행, 실습은 최대 3개로 제한한다.
- 스크롤·텍스트 넘침·요소 겹침을 허용하지 않는다.
- 구조도와 아이콘은 CSS 픽셀아트로 표현할 수 있다.
- 전체 재생성: `.venv\Scripts\python.exe _regen_slides.py --all`
...
상세 지침: `directives/curriculum.md`
...
| `tools/expert_feed_sync.ps1` | 유튜브 전문가 RSS 자동 연결 백필 래퍼 (작업 스케줄러 `Aihelper-ExpertFeed`, 매주 월 09:15) |
