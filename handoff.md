...
- **발표 자료**: 앱 커리큘럼은 1280×1600(4:5) 화면 슬라이드, 외부 파일은 Canva MCP 또는 `tools/pptx_maker.py`.

### UI (2026-06-27 개편)

- **보라색 SaaS 테마**: 액센트 `#7C5CFC`, 연보라 캔버스, 16px 둥근 카드, 그라데이션 히어로 배너(`_hero()`).
- **좌측 세로 네비로 일원화**: 상단 세그먼트 제거 → 사이드바가 메인 메뉴(`_render_sidebar_nav`). 활성 항목 연보라 강조. 다크모드 토글·역할·로그아웃은 하단.
- **수업 슬라이드 덱**: 4:5 캔버스를 여백 없이 활용하며, 간결한 시각 카드·픽셀아트 구조도·고정 크기 표로 구성. 스크롤과 텍스트 넘침을 금지한다.
